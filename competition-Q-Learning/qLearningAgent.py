...
def featurelize(state):
    ...
    def leftBlocked():
        return "West" in actions

    def rightBlocked():
        return "East" in actions

    def upBlocked():
        return "North" in actions

    def downBlocked():
        return "South" in actions

    return [
        len(state.getPellets()) / 100.0,
        nearestPellet() / 50.0,
        nearestGhostDistance() / 50.0,
        ...
        1.0 if leftBlocked() else 0.0,
        1.0 if rightBlocked() else 0.0,
        1.0 if upBlocked() else 0.0,
        1.0 if downBlocked() else 0.0
    ]
...
class CompetitionAgent(Agent):
    ...
    def updateWeight(self, reward, state, action, successor):
        ...
        print state.getScore(), len(self.w)
